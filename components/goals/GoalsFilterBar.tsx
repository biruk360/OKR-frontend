...
export default function GoalsFilterBar({ filters, onFilterChange }: GoalsFilterBarProps) {
  ...
  useEffect(() => {
    // Fetch timeframes and labels
    Promise.all([
      fetch('/api/timeframes').then(r => r.json()).catch(() => ({ success: false, data: [] })),
      fetch('/api/labels').then(r => r.json()).catch(() => ({ success: false, data: [] }))
    ]).then(([timeframesData, labelsData]) => {
