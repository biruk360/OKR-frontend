...
const CompanyRootNode = memo(({ data, selected }: NodeProps<CompanyRootNodeData>) => {
  ...
  return (
    <div
      className={`flex min-w-[140px] max-w-[220px] flex-col items-center rounded-xl border bg-white px-4 py-3 shadow-md ${
    ...
    >
      ...
      <p className="mt-2 text-center text-[11px] font-semibold uppercase tracking-wide text-gray-500">
        Company
      </p>
      <h3 className="mt-0.5 text-center text-sm font-bold leading-tight text-gray-900">{label}</h3>
      {timeframeName ? (
        <p className="mt-1 text-center text-[10px] text-gray-500">{timeframeName}</p>
      ) : null}
      <p className="mt-2 text-center text-[10px] tabular-nums text-gray-600">
