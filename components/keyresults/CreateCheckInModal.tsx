  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (tdRes.ok) {
        ...
        setTodos((d.todos ?? []).map((t: any) => ({ id: t.id, title: t.title, status: t.status })))
      } else {
        setTodos([])
      }
    } catch {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-5xl max-h-[95vh] overflow-y-auto border border-gray-200">
          <div className="sticky top-0 z-10 flex items-start justify-between gap-4 px-6 py-4 border-b border-gray-200 bg-white/95 backdrop-blur">
            <h2 className="text-base sm:text-lg font-semibold text-gray-900 leading-snug pr-8">
              {kr.title}
            </h2>
            <button
              type="button"
