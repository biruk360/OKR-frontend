...
| Project Management & Delivery Intelligence | IN PROGRESS | `features/projects/`, `lib/projects/`, `app/api/projects/`, `app/dashboard/projects/`, `app/portal/` | `ProjectViewSwitcher`, `GanttChart`, `DelayLedgerTable`, `ChartWrapper`, `ProjectChartsLibrary`, `ClientReportsPanel`, `PortfolioWbrPanel`, `PortfolioDashboard`, `PortfolioChartsLibrary`, `PortfolioReportPanel`, `PerformanceReportsPanel`, `ManagementReportsPanel`, `AiAssistantPanel`, `ProjectObjectiveLinker`, `MilestoneKeyResultLinker`, `ObjectiveDeliveryPanel`, `TemplateListClient`, `TemplateBuilderClient`, `NewProjectEntry`, `CreationDraftShell`, `CreateProjectWizard`, `ImportTemplateDownloads`, `ImportUploadStep`, `ColumnMappingStep`, `ValidationReportPanel`, `DraftReviewWorkspace`, `ChangeListPanel`, `CommitConfirmDialog` | Existing P1-P8 module and all 6 PM crons remain complete. Project Creation v1.1 P1 is complete. P2 Stories 2.1–2.4 now provide secure private retained uploads, optional strict/rate-limited OpenAI column proposals, explicit cleanup decisions, and ordered DOCX heading/paragraph/table extraction with persistent source references. Document content is plain bounded untrusted data, active/external content is disabled, and the fixed AI-facing framing cannot be overridden by document instructions. AC7, AC10, AC11, and AC13 are complete; AC5 awaits Story 2.5 DOCX template and AC12 awaits Story 2.6 provenance/assumptions. |
