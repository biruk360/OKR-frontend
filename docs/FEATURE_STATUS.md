...
| Project Management & Delivery Intelligence | IN PROGRESS | `features/projects/`, `lib/projects/`, `app/api/projects/`, `app/dashboard/projects/`, `app/portal/` | `ProjectViewSwitcher`, `GanttChart`, `DelayLedgerTable`, `ChartWrapper`, `ProjectChartsLibrary`, `ClientReportsPanel`, `PortfolioWbrPanel`, `PortfolioDashboard`, `PortfolioChartsLibrary`, `PortfolioReportPanel`, `PerformanceReportsPanel`, `ManagementReportsPanel`, `AiAssistantPanel`, `ProjectObjectiveLinker`, `MilestoneKeyResultLinker`, `ObjectiveDeliveryPanel`, `TemplateListClient`, `TemplateBuilderClient`, `NewProjectEntry`, `CreationDraftShell`, `CreateProjectWizard`, `ImportTemplateDownloads`, `ImportUploadStep`, `ColumnMappingStep`, `ValidationReportPanel`, `DraftReviewWorkspace`, `ChangeListPanel`, `CommitConfirmDialog` | Existing P1-P8 module and all 6 PM crons remain complete. Project Creation v1.1 P1 is complete. P2 Stories 2.1–2.4 provide secure retained uploads, optional OpenAI mapping, cleanup decisions, and ordered DOCX extraction. The Manual modal now has repaired responsive progress controls and type-first template selection for Website, Web Portal, Data Platform, Mobile App, Banking App, ICT Equipment Supply, and Import. Templates carry explicit type links; seven linked system schedules plus three general schedules are seeded on deployment and custom links are managed in the template directory/builder. Authorized project managers and management roles can archive a project from Project settings through an explicit retained-data confirmation; the project leaves the active directory while its records and audit history remain. AC7, AC10, AC11, and AC13 are complete; AC5 awaits Story 2.5 and AC12 awaits Story 2.6. |
| AI Automations — P0/P1 + NL compiler | LIVE | `features/automations/`, `lib/automations/`, `app/api/automations/`, `app/dashboard/automations/`, `types/automations.ts` | `AutomationList`, `AutomationDetail`, `AutomationForm`, `BriefingView`, `automationsApi`, `executeRun`, `runAutomationsTick`, `computeNextRunAt`, `diffFindings` | P0 of `docs/AI_Automations_Requirements_v1.0.md`: schema, schedule engine, tick + pm2 worker (`npm run worker:automations`), `okr.query` tool, AI synthesis, Briefing rendering (app/email/text), finding diffing, DRY_RUN→REVIEW→AUTO distribution, permissions seed, UI. P1 adds `odoo.search` (read-only, three-layer model narrowing) on the extracted `lib/odoo/client.ts`. Finding promotion (FR-12), PDF/DOCX export (FR-13), month-to-date spend (FR-17), the admin settings page (FR-18), the run-transcript viewer and the briefings list are all in. `npm run smoke:automations` drives the whole pipeline against a throwaway Postgres (34/34 checks: tick, exactly-once slots, SKIP LOCKED claiming, lease heartbeat, briefing production, run-to-run diffing, reaper). P2a adds the natural-language compiler (`POST /api/automations/compile`) with a two-attempt repair pass and a grouped plan diff. **Proven in production 2026-09-21:** an automation fired on its own schedule on the VPS — trigger `SCHEDULE`, slot hit exactly as `computeNextRunAt` predicted, `okr.query` 20 rows in 26 ms, a real OpenAI call ($0.0628), all three renderings produced, and **zero** recipient rows because DRY_RUN held. A permanent self-test fixture ("Self-test — scheduler health check") is ENABLED with `nextRunAt: null` so it never self-fires; `TestRunPanel` gives it a **Run test run** button, and `npm run automations:test-schedule` re-arms or removes it. **Deferred:** `web.search`/`web.fetch` + source registry (P2b — provider chosen (Tavily) but the tools stay out of `AVAILABLE_TOOL_IDS` until their SSRF review closes), credential vault + site login + mailbox (P3), Telegram channel, chaining and event triggers (P4). **Still unexercised:** a real Odoo call, and browser QA of the authoring form. |
...
| Authentication | DONE | `app/auth/`, `features/auth/`, `lib/auth.ts`, `app/api/auth/change-password/`, `app/api/wallpaper/` | Sign-in, sign-up, session, change password, 4-hour idle timeout; `SignInScreen`, `AuthBackdrop`, `AuthHero`, `SignInForm`, `useWallpaper` | NextAuth Credentials provider, JWT; `maxAge` 14400 s; client-side `useIdleTimeout` hook with 5-min warning toast. Sign-in rebuilt 2026-09-18 as a `features/auth` module: full-bleed Bing image-of-the-day backdrop (rotates per visit, `AUTH_WALLPAPER_SOURCE=off` for egress-restricted hosts), glass card on `react-hook-form`, caps-lock hint, remembered email. Sign-up / forgot-password still use the old centred card. |
...
| Real-time Notifications | DONE | `lib/pusher.ts`, `/api/pusher/auth`, `lib/stores/notification-store.ts` | Private per-user channel, live bell | `PUSHER_EVENTS.NOTIFICATION_SENT` is now triggered by both write paths (`dispatcher.ts` and `notifications/direct.ts`) and bound in `Header.tsx`. Needs real Pusher credentials — `isUsableCred()` rejects the placeholders in `env.example`, and the bell falls back to mount-and-open refresh when realtime is unavailable. |
...
| Preference gate for direct writers | DONE | `lib/notifications/direct.ts` | `writeDirectNotifications()` | Six writers (comments, letters, travel, automation briefings, both request-check-in routes) had their own event vocabularies and called `prisma.notification.create*` directly, bypassing preferences entirely — a user who switched a category off still received every one. They now share this gate. `grep prisma.notification.create` outside `lib/notifications/` returns nothing. |
| Email Templates | DONE | `lib/email/templates/index.ts` | `renderTemplate(eventKey, data)` | One template per event; redaction-aware |
| Notification Preferences | DONE | `app/dashboard/settings/notifications/`, `/api/notifications/preferences` | Per-category in-app/email/cadence | Account category is mandatory. **15 categories** — `TRAVEL`, `LETTER` and `AUTOMATION` added 2026-09-21. TRAVEL rows had been written all along under a category that was not in `ALL_CATEGORIES`, so travel notifications could not be switched off by anyone; letters and automations were filed under `ADMIN`, so muting admin digests muted them too. |
...
| Notification Cron Jobs | DONE | `lib/notifications/jobs.ts`, `/api/cron/notifications` | Digest drain, escalation, todo reminders, timeframe watcher, admin digests | **Now actually scheduled** (2026-09-18). `scripts/install-crontab.sh` installed 6 jobs; the other 21 — including all 8 notification jobs — lived only in `deploy/notifications-crontab.example`, which nothing installed, so the digest queue never drained. Installer is now the single source of truth; the example file is superseded. See `docs/CRON.md`. |
...
| Trello-Parity Sprint Board | IN PROGRESS | `docs/trello_parity_sprint_board_REQUIREMENTS.md`, `components/ui/popover.tsx`, `components/shared/LiveAnnouncer.tsx` | `Popover`, `LiveAnnouncer`, `announce()`, `sprintEditGuard` | **Phase 1 (foundations) DONE:** Popover primitive, aria-live announcer, global prefers-reduced-motion, token-system doc, Backlog `?noSprint=1` fix, column-route permission guard, watcher badge wired end-to-end, checklist item buttons wired. **Phase 2 (dynamic lists) DONE:** `SprintColumn` drives the board, `Todo.columnId`, add/rename/remap/archive lists, transactional reorder writing status, regression paths (AI, close, clone) wired. Needs `preflight.sql` + `prisma db push` before deploy. **Phase 3 (card visuals) DONE:** shared `CARD_PALETTE` + `--ap-card-*` tokens, label strip on cards, colour-blind patterns behind a per-user preference, BAND/FULL covers with luminance-picked ink, attachment/description badges. **Phase 4 (card modal) DONE:** migrated onto the shared `Modal` so it finally has a focus trap, list chip, mark-complete, overflow menu, comment edit/delete, Hide-details rail, read-only on closed sprints. **Phase 5 (share) DONE:** `POST /api/todos/[id]/share` + `?card=` deep link, authenticated only, no tokens; callbackUrl now preserves the target through sign-in (SHR-6). **Phase 6 (a11y) PARTIAL:** keyboard card movement (lift/move/drop/cancel with live announcements) added as a parallel path to pointer drag; lanes and cards carry list/listitem semantics; icon-only controls named. **Remaining:** dnd-kit migration of *both* drag paths (prerequisite for PRF-4 virtualisation, which would otherwise corrupt drop positions), CDM-9 label rename/recolor (blocked on assumption A3 — needs ADMIN-only label PATCH loosened), **`DTE-*` all DONE** — DTE-5 recurring shipped 2026-09-18 (`lib/todos/recurrence.ts` engine + `app/api/cron/todo-recurrence` generator + `Todo.recurrenceRule/recurrenceEndsAt/recurrenceParentId`); previously deferred and rendered disabled. Reminder select + `todo-reminders` cron every 5 min, validation, sprint-window warning, Remove clears everything. Custom Fields still deferred. |
...
| Notifications | DONE | `app/dashboard/notifications/`, `/api/notifications` | List, mark-read, mark-all-read, dismiss | Read API added 2026-09-18 (`GET /api/notifications`, `PATCH /api/notifications/[id]`, `POST /api/notifications/mark-all-read`). Before that the table was write-only over HTTP: the header bell said "No preview available yet", the page's "Mark all as read" button had no `onClick`, and `notification-store.ts` targeted three routes that did not exist. Bell, page and the sprint Inbox now share `lib/stores/notification-store.ts`. |
| Notification Inbox (sprint board) | DONE | `features/sprints/components/SprintInboxView.tsx` | Feed + mark-read + unread badge on the dock | Replaced the "Inbox is coming soon" dead end. |
| Bulk actions (OKR Explorer) | DONE | `app/dashboard/okrs-all/OkrsAllClient.tsx` | Bulk archive / restore across objectives, KRs and initiatives | Selection machinery already existed and the bar said "Bulk actions are coming soon". Fans out to the existing per-entity routes at concurrency 4 rather than adding a batch endpoint, so each route keeps its own permission check, lock guard and in-transaction rollup. Partial success is reported per row. |
| Move objective / key result | DONE | `components/shared/MoveOkrModal.tsx` | Re-parent an objective, re-file a KR | Was a disabled "Move (coming soon)" item in both action menus. The objective API already accepted `parentObjectiveId` with cycle checking; `PATCH /api/keyresults/[id]` did **not** accept `objectiveId` and was extended — it now checks permission on both source and target, requires the same timeframe, and recalculates the rollup for the old **and** new parent inside the mutation's transaction (invariant #9). |
| Data export (self-service) | DONE | `/api/me/export`, `app/dashboard/settings/account/` | User's own objectives, KRs, to-dos, check-ins, comments as JSON | |
| Account deletion (self-service) | NOT BUILT | `app/dashboard/settings/account/` | — | Shown disabled and labelled. Needs an ownership/retention policy for objectives, check-ins and audit rows owned by the deleted user — a policy call, not a missing handler. |
