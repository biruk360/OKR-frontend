...
| Real-time Notifications | PARTIAL | `lib/pusher.ts`, `lib/stores/notification-store.ts` | Pusher integration | `PUSHER_EVENTS.NOTIFICATION_SENT` is declared but never triggered or bound — the dispatcher does not import Pusher. The bell refreshes on mount and on open, not in real time. |
