...
- **Notes:** Project Creation v1.1 P1 is complete through Story 1.10. P2 Stories 2.1–2.4 add secure retained uploads, optional OpenAI column mapping, explicit cleanup decisions, and ordered DOCX source extraction: fail-closed file controls, strict/minimum AI mapping data, immutable cleanup evidence and decisions, Mammoth heading/paragraph/table order, durable source references, bounded plain text, and a fixed `UNTRUSTED_PROJECT_DATA` prompt boundary that document instructions cannot override. Mapping/cleanup/document content cannot silently mutate draft values or create a project; AC7, AC10, AC11, and AC13 are complete. Story 2.5 has not started. Projects remain `PLANNING` and unbaselined with no notification, portal, Jira, assignment, client invitation, or external send.
