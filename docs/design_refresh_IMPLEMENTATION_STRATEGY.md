...
**The test is whether the boundary is load-bearing**, not whether the element is
technically a control. If a field is identifiable without its stroke — a distinct
fill at 3:1, or an always-visible icon and label — the border is decorative and
`--ap-border` is fine. If removing the stroke would leave you unable to see where
the control is, it is required to identify the component and needs
`--ap-border-strong`.

Worked example, since this came up: the header search field is `--ap-bg-sunken` on
an `--ap-bg-raised` header — a **1.04:1** fill difference. Nothing but the stroke
shows where it is, so it takes `--ap-border-strong` even though the design
specifies the lighter value (~1.2:1). Where the mock fails AA, the mock loses —
the same call already made on the palette in §2.
...
| 2 — primitives | **Done** 2026-09-17 | Claude | 4 built (`Eyebrow` non-mono default, `FilterSelect` over Radix select, `EntityPicker` promoted from `LinkToOkrPopover`, `SectionHeading` extracted); `hooks/useOkrOptions` consolidates the two fetch strategies; `Popover` width+variant+shadow ramp, `ScrollArea` orientation, `Modal` `940`+`accentColor`; `MiniBadge` promoted with tone/mono; `Progress` retargeted (call sites **not** migrated); accordion/alert-dialog/toggle-group/toggle deleted. No `CountChip`, no `FloatingDock`. `DueDateChip` deferred — needs the vocabulary decision first. |
| 3 — shell | **Done** 2026-09-17 | Claude | **40px dead strip fixed** — aside + grid track now read one `--ap-sidebar-w` (228/52), grid rows 54px, plus a pre-paint boot script so collapsed users stop getting a layout jump. Sidebar white with 5px dot markers, mono eyebrows, 32px/7px rows; all 14 permission-gated groups intact. Header 54px, centred 420px search, mono ⌘K. Two calls: header went **solid not glass** (nothing scrolls beneath it, so `backdrop-filter` had nothing to blur); **fake notifications removed** rather than restyled — no `/api/notifications` list route exists, so badge + 3 hardcoded strings deleted in favour of an honest empty state. **Resolved:** the search field keeps `--ap-border-strong`. Its fill differs from the header by 1.04:1, so the stroke is the only thing identifying the control — 1.4.11 applies. Rationale recorded inline in `Header.tsx` and as a worked example in §2. |
| 4 — sprint board | **Done** 2026-09-17 | Claude | Lanes 286/10px/8px gap + `calc(100vh - 340px)` scroller; cards 10px + 5px cover + 21px meta chip row + specified hover; empty lane §4.1 dashed panel; segmented filter; "Add another list" 218×44. URGENT stripe's literal second orange → `color-mix` on `--ap-warn`; list-manager + planner lanes moved off shadcn `border`; `PlannerTimeGrid` now reads `todoStatusMeta()`. **Dark fork finished** — dock, planner, time grid, cards, mobile strip, bg-picker trigger, header chrome (dock active pill was 4.0:1, now 5.0:1 on a dark fill with ~10:1 inactive ink). Deferred per Decision 0: per-lane header `+`. `tsc` clean, `test:sprints` 21/21. **No browser check yet.** |
| 5 — card modal | **Done** 2026-09-17 | Claude | 940px shell, top-aligned (not centred), 6px status accent strip, `minmax(0,1fr) 232px` with both columns scrolling at `calc(100vh-140px)`. Absolute header row deleted — complete-toggle inline at 26px, so `pt-16` and the banner's `mt-4`/`mt-14` flip went with it. Metadata line + `auto-fit minmax(192px,1fr)` attribute grid (Members / Labels×2 / Due / Priority). **~70 `font-5/6/700` classes were invalid Tailwind v3 and rendered nothing** — now real weights. `mode="drawer"` branch, its portal, `isDrawer` and its never-firing Escape listener deleted (prop kept, narrowed + deprecated, for one call site). `DatesPanel`'s duplicate close button removed; `deleteLabel` off `window.confirm` onto `ConfirmDialog`; `PRIORITY_COLORS` + `TodoCard`'s copy retargeted to tokens; `TodoCard` avatars onto `lib/user-color`. Deferred per §6.4: tabs, named checklists, Hide checked, comment toolbar, Reply/React, card-ID chip. Overlay scrim **closed** — `DialogOverlay` in `components/ui/dialog.tsx` now carries `.ap-modal-overlay`, so `--ap-overlay` + `blur(3px)` applies to all ~50 Modal consumers (the old flat `bg-black/10` was nearly invisible on the refreshed light surfaces and far too weak in dark mode). `tsc` clean, `test:cards` 9/9, `test:security` 20/20. **No browser check yet.** |
